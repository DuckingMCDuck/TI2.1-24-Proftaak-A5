...
namespace HealthyFromHomeApp.Clients
{
    internal class VRServer
    {
        public VRServer() 
        {
            ...
            {
                ClientMainWindow.client.chatText = "Connected to VR Server!\n";
                ...
                while (true)
                {
                    ...
                    {
                        ClientMainWindow.client.chatText = "Sending packets...";
                        ...
                        ClientMainWindow.client.chatText = "Sending packet...";
                        stream.WriteAsync(finalPacket, 0, finalPacket.Length);

                        ClientMainWindow.client.chatText = "Packet sent. Waiting for response...";

                        // Optional: Read response from the server
                        byte[] buffer = new byte[1024];
                        int bytesRead = stream.Read(buffer, 0, buffer.Length);
                        string response = Encoding.UTF8.GetString(buffer, 0, bytesRead);
                        ClientMainWindow.client.chatText = "Received response: " + response;
                    }
                }
            }
        }
