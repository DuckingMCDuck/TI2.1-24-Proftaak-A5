            ...
            try
            {
                ...
                else if (difDataInt[4] == 25)
                {
                    ...
                    for (int i = 0; i < difDataInt.Count; i++)
                    {
                        ...
                        else if (name == "Accumulated_Power_MSB" && lsbValue != 0)
                        {
                            msbValue = difDataInt[i];
                            calcPower = true;
                        }
                        ...
                        if (calcPower == true)
                        {
                            double power = ((msbValue << 8) | lsbValue) / 1000 * 3.6;
                            (string, int) powerTuple = ("AccumulatedPower", (int)Math.Round(power));
                            dataWithNames.Add(powerTuple);
                            calcPower = false;
                        }
                    }
                }
